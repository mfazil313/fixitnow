...
import { WrenchIcon, GoogleIcon, UserIcon } from '@/components/Icons';
...
export default function CustomerSignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const userId = data?.user?.id || `customer-${Date.now()}`;
    ...
    if (typeof window !== 'undefined') {
      window.localStorage.setItem('fixitnow_user', JSON.stringify({
        id: userId,
        email: cleanEmail,
        user_metadata: { full_name: fullName, role: 'customer', phone }
      }));
    }
      ...
      <div className={styles.page}>
        <div className={styles.card}>
          <div className={styles.successState}>
            ...
            <p>Welcome to FixItNow. Redirecting to upload problem page...</p>
          </div>
        </div>
      ...
      <div className={styles.card}>
        <div className={styles.header}>
          ...
          <div className={styles.roleToggle} style={{ width: '100%', margin: '4px 0 8px 0' }}>
            ...
            <Link href="/auth/signup/worker" className={styles.roleBtn}>
              <WrenchIcon size={16} /> {t('auth.service_pro_worker')}
            </Link>
          </div>

          <h1 className={styles.title}>{t('auth.register_as_customer')}</h1>
          <p className={styles.subtitle}>{t('auth.customer_subtitle')}</p>
        </div>
        ...
        <form onSubmit={handleSignup} className={styles.form}>
          <div className="form-group">
            ...
            <div className="form-input-icon">
              ...
              <input type="text" className="form-input" placeholder="e.g. Priya Mehta" value={fullName} onChange={e => setFullName(e.target.value)} required />
            </div>
          </div>
          ...
          <div className="form-group">
            ...
            <div className="form-input-icon">
              ...
              <input type="tel" className="form-input" placeholder="e.g. +91 98765 43210" value={phone} onChange={e => setPhone(e.target.value)} required />
            </div>
          </div>
          ...
          <button type="submit" className="btn btn-primary btn-full btn-lg" disabled={loading} style={{ marginTop: '8px' }}>
            {loading ? <><span className="spinner" />{t('auth.creating_account')}</> : t('auth.create_customer_account')}
          </button>
        </form>
